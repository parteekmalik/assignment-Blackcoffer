...
function MainNav({ navigation }: { navigation: NestedList[] }) {
    ...
    const { modifyPath, currNavigation, isPathOpened } = useContext(NavigationCotext);
        ...
        if (Array.isArray(item)) {
            ...
            return (
                <li key={depth + link + index}>
                    <span className=" text-lightBlack  p-[0px_12px] m-[16px_12px_6px] text-custom-sm opacity-[.4]">{item.name}</span>
                </li>
            );
        return (
...
function ExpandableNav({ icon, isOpened, children, headTitle, onClick, isSelected }: { isOpened: boolean; icon: IconType; onClick?: MouseEventHandler<HTMLDivElement>; children: ReactNode | ReactNode[]; headTitle: string; isSelected: boolean }) {
    return (
        <DefaultLayout onClick={onClick} ulChildren={{ list: children, isvisible: isOpened }} isSelected={isSelected}>
            <IconSVGs icon={icon} />
...
function NavLink(props: { Name: string; onClick?: MouseEventHandler<HTMLDivElement>; depth: number; link: string; isSelected: boolean }) {
    const { pathname } = useLocation();

    return (
        <Link to={props.link}>
            <DefaultLayout isSelected={props.isSelected} onClick={props.onClick}>
                <DefaultSVG marginRight={13} marginLeft={5 + 10 * props.depth} isSelected={props.isSelected} />
                <span className={!props.isSelected ? "text-lightBlack opacity-[.9]" : "text-white"}>{props.Name}</span>
            </DefaultLayout>
        </Link>
    );
}

function DefaultLayout({ children, ulChildren, isSelected, onClick }: { isSelected?: boolean; children: ReactNode; ulChildren?: { list: ReactNode; isvisible: boolean }; onClick?: MouseEventHandler<HTMLDivElement> }) {
    return (
        <li>
            <div onClick={onClick} className={"cursor-pointer rounded-lg  flex items-center p-[0px_12px] m-[0px_12px_6px] h-[44px] " + " text-lightBlack opacity-[.9] " + (isSelected ? " bg-main-purple text-white " : " hover:bg-hoverColor ")} style={{ width: "cal(100%-24px)" }}>
                {children}
            </div>
            {ulChildren && <ul style={ulChildren.isvisible ? {} : { display: "none" }}>{ulChildren.list}</ul>}
        </li>
    );
}
