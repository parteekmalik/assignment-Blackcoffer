    ...
    return (
        <aside
            className={
                ...
                (deviceType === "desktop" ? "  " : isNavOpened ? " w-screen " : " translate-x-[-260px] ")
                // +' desktop:w-auto laptop:w-screen tablet:w-screen mobile:w-screen'
            }
        >
            <div className={"w-[260px] z-30 flex flex-col border-r h-full border-black bg-white "}>
                <div className="flex gap-2 p-[20px_2px_20px_8px] items-end mx-[12px]">
                    <LogoSVG />
                    <span className="text-xl  font-semibold">Vuexy</span>
                    <div onClick={() => setisNavOpened!(false)} className="desktop:hidden ml-auto">
                        <CloseSVG />
                    </div>
                </div>
